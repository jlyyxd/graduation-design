...
class ImgDataSets(Dataset):
    ...
    def __getitem__(self, idx):
        case = self.sample_list[idx]
        base_dir = os.path.join(self._base_dir, "data", "HCMS", self.split)
        image_path = os.path.join(base_dir, "image", "{}.png".format(case))
        label_path = os.path.join(base_dir, "label", "{}.png".format(case))

        if self.split == "train":
            img_type = cv2.IMREAD_GRAYSCALE # 训练集以灰度读入
        else:
            img_type = cv2.IMREAD_COLOR # 非训练集以彩图读入
        image = cv2.imread(image_path, img_type)
        label = cv2.imread(label_path, img_type)
        sample = {"image": image, "label": label}
        if self.split == "train":
            if None not in (self.ops_weak, self.ops_strong):
                sample = self.transform(sample, self.ops_weak, self.ops_strong)
