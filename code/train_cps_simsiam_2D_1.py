...
parser.add_argument('--model', type=str,
                    default='unet_simsiam', help='model_name')
...
def patients_to_slices(dataset, patiens_num):
    ...
    if "ACDC" in dataset:
        ...
        ref_dict = {"68": 3, "136": 7,
                    "256": 14, "396": 21, "512": 28, "664": 35, "1312": 140}
    elif "Prostate":
        # ref_dict = {"2": 27, "4": 53, "8": 120,
        #             "12": 179, "16": 256, "21": 312, "42": 623}
        ref_dict = {"27": 2, "53": 4, "120": 8,
                    "179": 12, "256": 16, "312": 21, "623": 42}
    else:
        print("Error")
...
def train(args, snapshot_path):
    ...
    model1 = create_model()# 创建模型
    model2 = create_model()# 创建模型
    
    def worker_init_fn(worker_id):
        random.seed(args.seed + worker_id)

# 建立数据集,在 /data/ACDC/ 下面
    db_train = BaseDataSets(base_dir=args.root_path, split="train", num=None, transform=transforms.Compose([
        RandomGenerator(args.patch_size)
    ]))# train数据集
    db_val = BaseDataSets(base_dir=args.root_path, split="val")# val数据集
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            p1, z1, outputs1 = model1(volume_batch)
            outputs_soft1 = torch.softmax(outputs1, dim=1)

            p2, z2, outputs2 = model2(volume_batch)
            ...
            loss3 = -(criterion(p1, z2).mean() + criterion(p2, z1).mean) * 0.5

            # 总loss
            loss = model1_loss + model2_loss + loss3
            ...
            writer.add_scalar('loss/CosineSimilarity',
                              loss3, iter_num)
