...
class UNet_SIMSIAM(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        # 添加projector和predictor
        feature_chns = params['feature_chns'][-1]
        projection_chns = 128
        self.projector = nn.Sequential(nn.Linear(feature_chns, feature_chns, bias=False),
                                       nn.BatchNorm1d(feature_chns),
                                       nn.ReLU(inplace=True),# 第一层
                                       nn.Linear(feature_chns, feature_chns, bias=False),
                                       nn.BatchNorm1d(feature_chns),
                                       nn.ReLU(inplace=True),# 第二层
                                       nn.Linear(feature_chns, projection_chns, bias=False),
                                       nn.BatchNorm1d(projection_chns, affine=False)# 第三层
                                       )

        self.predictor = nn.Sequential(nn.Linear(projection_chns, feature_chns, bias=False),
                                       nn.BatchNorm1d(feature_chns),
                                       nn.ReLU(inplace=True),# 隐藏层
                                       nn.Linear(feature_chns, projection_chns)# 输出层
                                       )
    ...
    def forward(self, x):
        feature = self.encoder(x)
        feature_projection = self.projector(feature[-1])
        prediction = self.predictor(feature_projection)
        output = self.decoder(feature)
        return prediction, feature_projection.detach(), output
